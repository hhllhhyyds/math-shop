...
fn fft_local<F: FloatTraitsForComplex>(data: &mut [Complex<F>], temp: &mut [Complex<F>]) {
    let n = data.len();
    if n > 1 {
        (0..(n / 2)).for_each(|j| {
            temp[j] = data[j] + data[j + n / 2];
            temp[j + n / 2] = (data[j] - data[j + n / 2]) * unit_complex(n as u64, -(j as i64));
        });

        fft_local(&mut temp[..(n / 2)], &mut data[..(n / 2)]);
        fft_local(&mut temp[(n / 2)..], &mut data[(n / 2)..]);
...
fn ifft_local<F: FloatTraitsForComplex>(data: &mut [Complex<F>], temp: &mut [Complex<F>]) {
    let n = data.len();
    if n > 1 {
        (0..(n / 2)).for_each(|j| {
            temp[j] = data[j] + data[j + n / 2];
        ...
        ifft_local(&mut temp[..(n / 2)], &mut data[..(n / 2)]);
        ifft_local(&mut temp[(n / 2)..], &mut data[(n / 2)..]);
